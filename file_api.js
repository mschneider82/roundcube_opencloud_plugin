...
function files_api()
{
    ...
    {
        if (typeof p == 'string') { this.translations[p] = value; }
        else if (typeof p == 'object') { $.extend(this.translations, p); }
    };

    // return a localized string
    this.t = function (label)
    {
        if (this.translations[label]) { return this.translations[label]; }
        return label;
    };
    ...
    this.http_error = function (request, status, err)
    {
        ...
        if (request.status && errmsg) { this.display_message(this.t('servererror') + ' (' + errmsg + ')', 'error'); }
    };
    ...
    this.response_parse = function (response)
    {
        if (!response || response.status != 'OK') {
            ...
            if (response && response.code == 403) { this.logout(response); }
            else { this.display_message(response && response.reason ? response.reason : this.t('servererror'), 'error'); }
    ...
    {
        if (size >= 1073741824) { return parseFloat(size / 1073741824).toFixed(2) + ' GB'; }
        if (size >= 1048576) { return parseFloat(size / 1048576).toFixed(2) + ' MB'; }
    ...
    this.file_type_supported = function (type)
    {
        ...
        if (caps.tif) { img += '|tiff'; }

        if ((new RegExp('^image/(' + img + ')$', 'i')).test(type)) { return 1; }

        // prefer text viewer for any text type
        if (/^text\/(?!(pdf|x-pdf))/i.test(type)) { return 2; }

        if (caps.pdf) {
            regexps.push(/^application\/(pdf|x-pdf|acrobat|vnd.pdf)/i, /^text\/(pdf|x-pdf)/i);
        }

        if (caps.flash) { regexps.push(/^application\/x-shockwave-flash/i); }

        for (i in regexps) { if (regexps[i].test(type)) { return 1; } }

        for (i in navigator.mimeTypes) {
            t = navigator.mimeTypes[i].type;
            if (t == type && navigator.mimeTypes[i].enabledPlugin) { return 1; }
        }

        // types with viewer support
        if ($.inArray(type, this.env.supported_mimetypes) > -1) { return 2; }
    };
    ...
    this.browser_capabilities = function ()
    {
        ...
        for (i in ctypes) { if (this.env.browser_capabilities[ctypes[i]]) { caps.push(ctypes[i]); } }
    ...
    {
        if (!this.env.browser_capabilities) { this.env.browser_capabilities = {}; }

        if (this.env.browser_capabilities.pdf === undefined) { this.env.browser_capabilities.pdf = this.pdf_support_check(); }

        if (this.env.browser_capabilities.flash === undefined) { this.env.browser_capabilities.flash = this.flash_support_check(); }

        if (this.env.browser_capabilities.tif === undefined) { this.tif_support_check(); }
    };
    ...
    this.pdf_support_check = function ()
    {
        ...
        if (plugin && plugin.enabledPlugin) { return 1; }

        if (window.ActiveXObject) {
            try {
                if (axObj = new ActiveXObject('AcroPDF.PDF')) { return 1; }
            }
            catch (e) {}
            try {
                if (axObj = new ActiveXObject('PDF.PdfCtrl')) { return 1; }
            }
            catch (e) {}
        }
        ...
        for (i = 0; i < len; i++) {
            ...
            if (typeof plugin === 'String') {
                if (regex.test(plugin)) { return 1; }
            }
            else if (plugin.name && regex.test(plugin.name)) { return 1; }
        }
    ...
    this.flash_support_check = function ()
    {
        ...
        if (window.ActiveXObject) {
            try {
                if (axObj = new ActiveXObject('ShockwaveFlash.ShockwaveFlash')) { return 1; }
            }
            catch (e) {}
        }
