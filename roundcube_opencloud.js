    ...
    if (rcmail.task == 'mail') {
        if (rcmail.env.action == 'compose') {
            ...
            var input = $(`<button class="btn btn-secondary attach cloud" type="button" tabindex="${$('button', elem).attr('tabindex') || 0}">
                ${rcmail.gettext('roundcube_opencloud.fromcloud')}
            </button>`).click(function () { roundcube_opencloud_selector_dialog(); });
            ...
            if (rcmail.gui_objects.filelist) {
                ...
                rcmail.file_list.addEventListener('select', function (o) { roundcube_opencloud_list_select(o); })
                ...
                roundcube_opencloud_list_coltypes();
            }
        ...
        // mail preview
            ...
            if (header_links.length) {
                header_links.append(
                    $(`<a href="#" class="button filesaveall">${rcmail.gettext('roundcube_opencloud.saveall')}</a>`)
                        .on('click', function () { roundcube_opencloud_directory_selector_dialog(); })
                );
            }

            rcmail.addEventListener('menu-open', roundcube_opencloud_attach_menu_open);
        ...
        roundcube_opencloud_init();
    }
    else if (rcmail.task == 'roundcube_opencloud') {
        if (rcmail.gui_objects.filelist) {
            rcmail.file_list = new rcube_list_widget(rcmail.gui_objects.filelist, {
                multiselect: true,
                draggable: true,
            ...
            /*
            rcmail.file_list.row_init = function(o){ roundcube_opencloud_init_file_row(o); };
            ...
            */
            rcmail.file_list.addEventListener('dblclick', function (o) { roundcube_opencloud_list_dblclick(o); })
                .addEventListener('select', function (o) { roundcube_opencloud_list_select(o); })
                .addEventListener('keypress', function (o) { roundcube_opencloud_list_keypress(o); })
                .addEventListener('dragend', function (e) { roundcube_opencloud_drag_end(e); })
                .addEventListener('column_replace', function (e) { roundcube_opencloud_set_coltypes(e); })
                .addEventListener('listupdate', function (e) { rcmail.triggerEvent('listupdate', e); });
            ...
            rcmail.file_list.init();
            roundcube_opencloud_list_coltypes();
            roundcube_opencloud_drag_drop_init($(rcmail.gui_objects.filelist).parents('.droptarget'));
        }
...
function roundcube_opencloud_init()
{
    if (window.file_api) { return; }

    // Initialize application object (don't change var name!)
    file_api = $.extend(new files_api(), new roundcube_opencloud_ui());
...
function roundcube_opencloud_directory_selector_dialog(id)
{
    ...
    buttons[rcmail.gettext('roundcube_opencloud.save')] = function () {
    ...
    buttons[rcmail.gettext('roundcube_opencloud.cancel')] = function () {
        roundcube_opencloud_dialog_close(this);
    };
    ...
    // show dialog window
    roundcube_opencloud_dialog_show(dialog, {
        title: rcmail.gettext('roundcube_opencloud.' + label),
        buttons: buttons,
    ...
    if (rcmail.is_framed()) {
        if (!parent.rcmail.folder_create) {
            ...
            parent.rcmail.folder_create = function () {
                window.roundcube_opencloud_folder_create_dialog();
            };
        }
        ...
        if (!parent.rcmail.folder_force_reload) {
            ...
            parent.rcmail.folder_force_reload = function () {
                window.roundcube_opencloud_folder_force_reload();
            };
        }
    }
}
...
function roundcube_opencloud_selector_dialog()
{
    var dialog = $('#files-compose-dialog'), buttons = {};

    buttons[rcmail.gettext('roundcube_opencloud.attachsel')] = function () {
        var list = [];
        ...
        if (list.length) {
            ...
            rcmail.add2attachment_list(id, {
                name: '',
                html: `<span>${rcmail.get_label('roundcube_opencloud.attaching')}</span>`,
                classname: 'uploading',
                complete: false,
            });
    ...
    // show dialog window
    roundcube_opencloud_dialog_show(dialog, {
        title: rcmail.gettext('roundcube_opencloud.selectfiles'),
        buttons: buttons,
        button_classes: ['mainaction'],
        minWidth: 500,
        minHeight: 300,
...
function roundcube_opencloud_attach_menu_open(p)
{
    ...
    $('#attachmenusaveas').unbind('click').attr('onclick', '').click(function (e) {
        return roundcube_opencloud_directory_selector_dialog(id);
    });
}

// folder creation dialog
function roundcube_opencloud_folder_create_dialog()
{
    ...
    buttons[rcmail.gettext('roundcube_opencloud.create')] = function () {
    ...
    // show dialog window
    roundcube_opencloud_dialog_show(dialog, {
        title: rcmail.gettext('roundcube_opencloud.foldercreate'),
        buttons: buttons,
        button_classes: ['mainaction'],
    });

    // Fix submitting form with Enter
    $('form', dialog).submit(roundcube_opencloud_dialog_submit_handler);
...
// folder edit dialog
function roundcube_opencloud_folder_edit_dialog()
{
    ...
    // show dialog window
    roundcube_opencloud_dialog_show(dialog, {
        title: rcmail.gettext('roundcube_opencloud.folderedit'),
        buttons: buttons,
        button_classes: ['mainaction'],
    });
...
function roundcube_opencloud_folder_mount_dialog()
{
    var args = { buttons: {}, title: rcmail.gettext('roundcube_opencloud.foldermount') },
        dialog = $('#files-folder-mount-dialog'),
        input = $('#folder-mount-name').val('');

    args.buttons[rcmail.gettext('roundcube_opencloud.save')] = function () {
    ...
    args.buttons[rcmail.gettext('roundcube_opencloud.cancel')] = function () {
        roundcube_opencloud_dialog_close(this);
    };
    ...
    // show dialog window
    roundcube_opencloud_dialog_show(dialog, args, function () {
...
// file edition dialog
function roundcube_opencloud_file_edit_dialog(file)
{
    ...
    // show dialog window
    roundcube_opencloud_dialog_show(dialog, {
        title: rcmail.gettext('roundcube_opencloud.fileedit'),
        buttons: buttons,
        button_classes: ['mainaction'],
    });
}

function roundcube_opencloud_dialog_show(content, params, onopen)
{
    params = $.extend({
        modal: true,
        resizable: true,
...
function roundcube_opencloud_dialog_submit_handler()
{
...
// Hides dialog
function roundcube_opencloud_dialog_close(dialog)
{
    (rcmail.is_framed() ? window.parent : window).$(dialog).dialog('close');
}

function roundcube_opencloud_folder_force_reload()
{
    file_api.folder_list(true);
}

// smart upload button
function roundcube_opencloud_upload_input(button)
{
    var link = $(button),
        file = $('<input>'),
        offset = link.offset();
    ...
    file.attr({
        name: 'file[]', type: 'file', multiple: 'multiple', size: 5, title: link.attr('title'), tabindex: '-1',
    })
        .change(function () { rcmail.files_upload('#filesuploadform'); })
        .click(function () { setTimeout(function () { link.mouseleave(); }, 20); })
    // opacity:0 does the trick, display/visibility doesn't work
        .css({
            opacity: 0, cursor: 'pointer', outline: 'none', position: 'absolute',
        });
    ...
    if (bw.mz || bw.ie) { file.css({ marginLeft: '-80px' }); }
    ...
    // place button under the cursor
        .mousemove(function (e) {
            if (rcmail.commands['files-upload'] && this.__active) { move_file_input(e); }
...
roundcube_opencloud_list_coltypes = function ()
{
...
roundcube_opencloud_set_list_options = function (cols, sort_col, sort_order)
{
    var update = 0, i, idx, name, newcols = [], oldcols = rcmail.env.coltypes;

    if (sort_col === undefined) { sort_col = rcmail.env.sort_col; }
    if (!sort_order) { sort_order = rcmail.env.sort_order; }
    ...
    // make sure new columns are added at the end of the list
        ...
        for (i = 0; i < cols.length; i++) { if (cols[i]) { newcols.push(cols[i]); } }
    ...
    if (update == 1) { rcmail.command('files-list', { sort: sort_col, reverse: sort_order == 'DESC' }); }
        ...
        rcmail.http_post('files/prefs', {
            roundcube_opencloud_list_cols: oldcols,
            roundcube_opencloud_sort_col: sort_col,
            roundcube_opencloud_sort_order: sort_order,
        }, rcmail.set_busy(true, 'loading'));
    }
};

roundcube_opencloud_set_coltypes = function (list)
{
    var i, found, name, cols = list.list.tHead.rows[0].cells;

    rcmail.env.coltypes = [];

    for (i = 0; i < cols.length; i++) { if (cols[i].id && cols[i].id.match(/^rcm/)) {
    ...
    rcmail.http_post('files/prefs', { roundcube_opencloud_list_cols: rcmail.env.coltypes });
};

roundcube_opencloud_list_dblclick = function (list)
{
    rcmail.command('files-open');
};

roundcube_opencloud_list_select = function (list)
{
...
//      caps = this.browser_capabilities().join();
      href = '?' + $.param({_task: 'roundcube_opencloud', _action: 'open', file: file, viewer: viewer == 2 ? 1 : 0});
...
roundcube_opencloud_list_keypress = function (list)
{
    if (list.modkey == CONTROL_KEY) { return; }

    if (list.key_pressed == list.ENTER_KEY) { rcmail.command('files-open'); }
    else if (list.key_pressed == list.DELETE_KEY || list.key_pressed == list.BACKSPACE_KEY) { rcmail.command('files-delete'); }
};

roundcube_opencloud_drag_end = function (e)
{
...
roundcube_opencloud_drag_menu_action = function (command)
{
...
roundcube_opencloud_selected = function ()
{
    var files = [];
    $.each(rcmail.file_list.get_selection(), function (i, v) {
        var name, row = $('#rcmrow' + v);

        if (row.length == 1 && (name = row.data('file'))) { files.push(name); }
    });

    return files;
};

roundcube_opencloud_frame_load = function (frame)
{
    var win = frame.contentWindow;

    rcmail.file_editor = win.file_editor && win.file_editor.editable ? win.file_editor : null;

    if (rcmail.file_editor) { rcmail.enable_command('files-edit', true); }
    ...
    try {
        if ($('#fileframe').contents().find('#print').length) { rcmail.enable_command('files-print', true); }
    }
    catch (e) {}
};

// activate html5 file drop feature (if browser supports it)
roundcube_opencloud_drag_drop_init = function (container)
{
    ...
    container.bind('dragover dragleave', function (e) {
        return roundcube_opencloud_drag_hover(e);
    });
    ...
    container.get(0).addEventListener('drop', function (e) {
        ...
        roundcube_opencloud_drag_hover(e);
        return file_api.file_drop(e);
    }, false);
};

// handler for drag/drop on element
roundcube_opencloud_drag_hover = function (e)
{
    if (!file_api.env.folder) { return; }
    ...
    if (!elem.hasClass('droptarget')) { elem = elem.parents('.droptarget'); }

    elem[e.type == 'dragover' ? 'addClass' : 'removeClass']('hover');
};

// returns localized file size
roundcube_opencloud_file_size = function (size)
{
...
roundcube_opencloud_progress_str = function (param)
{
    ...
    total = roundcube_opencloud_file_size(param.total);
...
rcube_webmail.prototype.files_sort = function (props)
{
    ...
    this.http_post('files/prefs', { roundcube_opencloud_sort_col: sort_col, roundcube_opencloud_sort_order: sort_order });
...
{
    if (confirm(this.get_label('roundcube_opencloud.folderdeleteconfirm'))) { file_api.folder_delete(file_api.env.folder); }
};

rcube_webmail.prototype.files_delete = function ()
{
    if (!confirm(this.get_label('roundcube_opencloud.filedeleteconfirm'))) { return; }

    var files = this.env.file ? [this.env.file] : roundcube_opencloud_selected();
    file_api.file_delete(files);
};

rcube_webmail.prototype.files_move = function (folder)
{
    var files = roundcube_opencloud_selected();
...
{
    if (form) { file_api.file_upload(form); }
};
...
{
    if (this.file_editor && this.file_editor.printable) { this.file_editor.print(); }
...
{
    roundcube_opencloud_folder_create_dialog();
};

rcube_webmail.prototype.folder_rename = function ()
{
    roundcube_opencloud_folder_edit_dialog();
};

rcube_webmail.prototype.folder_mount = function ()
{
    roundcube_opencloud_folder_mount_dialog();
};
...
function roundcube_opencloud_ui()
{
    this.requests = {};
    this.uploads = [];
...
*/
    ...
    this.folder_list_response = function (response)
    {
        ...
        $.each(collections, function (i, n) {
            var row = $(`<li id="folder-collection-${n}" tabindex="0" class="mailbox collection ${n}">
                <span class="name">${rcmail.gettext(`roundcube_opencloud.collection_${n}`)}</span>
            </li>`);
    ...
    {
        this.req = this.set_busy(true, 'roundcube_opencloud.foldercreating');
    ...
    this.folder_create_response = function (response)
    {
        ...
        this.display_message('roundcube_opencloud.foldercreatenotice', 'confirmation');
    ...
    this.folder_rename = function (folder, new_name)
    {
        ...
        this.req = this.set_busy(true, 'roundcube_opencloud.folderupdating');
    ...
    this.folder_rename_response = function (response)
    {
        ...
        this.display_message('roundcube_opencloud.folderupdatenotice', 'confirmation');
    ...
    {
        this.req = this.set_busy(true, 'roundcube_opencloud.foldermounting');
    ...
    this.folder_mount_response = function (response)
    {
        ...
        this.display_message('roundcube_opencloud.foldermountnotice', 'confirmation');
    ...
    {
        this.req = this.set_busy(true, 'roundcube_opencloud.folderdeleting');
    ...
    this.folder_delete_response = function (response)
    {
        ...
        this.display_message('roundcube_opencloud.folderdeletenotice', 'confirmation');
    ...
    this.file_list_row = function (file, data, index)
    {
        ...
        $('td.options > span', row).click(function (e) {
            roundcube_opencloud_file_edit_dialog(file);
        });
    ...
    {
        this.req = this.set_busy(true, 'roundcube_opencloud.filedeleting');
    ...
    this.file_delete_response = function (response)
    {
        ...
        this.display_message('roundcube_opencloud.filedeletenotice', 'confirmation');
    ...
    this.file_move = function (files, folder)
    {
        ...
        this.req = this.set_busy(true, 'roundcube_opencloud.filemoving');
    ...
    this.file_move_response = function (response)
    {
        ...
        if (response.result && response.result.already_exist && response.result.already_exist.length) { this.file_move_ask_user(response.result.already_exist, true); }
            ...
            this.display_message('roundcube_opencloud.filemovenotice', 'confirmation');
            this.file_list();
        }
    };
    ...
    this.file_copy = function (files, folder)
    {
        ...
        this.req = this.set_busy(true, 'roundcube_opencloud.filecopying');
    ...
    this.file_copy_response = function (response)
    {
        ...
        if (response.result && response.result.already_exist && response.result.already_exist.length) { this.file_move_ask_user(response.result.already_exist); }
            ...
            this.display_message('roundcube_opencloud.filecopynotice', 'confirmation');
            this.quota();
        }
    };
    ...
    this.file_move_ask_user = function (list, move)
    {
        var file = list[0], buttons = {},
            text = rcmail.gettext('roundcube_opencloud.filemoveconfirm').replace('$file', file.dst);
        dialog = $('<div></div>');

        buttons[rcmail.gettext('roundcube_opencloud.fileoverwrite')] = function () {
            ...
            file_api.req = file_api.set_busy(true, move ? 'roundcube_opencloud.filemoving' : 'roundcube_opencloud.filecopying');
            file_api.request(action, { file: f, overwrite: 1 }, 'file_move_ask_user_response');
        };

        if (list.length > 1) { buttons[rcmail.gettext('roundcube_opencloud.fileoverwriteall')] = function () {
        ...
        buttons[rcmail.gettext('roundcube_opencloud.fileskip')] = skip_func;

        if (list.length > 1) { buttons[rcmail.gettext('roundcube_opencloud.fileskipall')] = function () {
            dialog.dialog('destroy').remove();
            if (move) { file_api.file_list(); }
        }; }

        // open jquery UI dialog
        roundcube_opencloud_dialog_show(dialog.html(text), {
            close: skip_func,
            buttons: buttons,
            minWidth: 400,
            width: 400,
    ...
    this.file_move_ask_user_response = function (response)
    {
        ...
        if (list && list.length) { this.file_move_ask_user(list, mode); }
            ...
            this.display_message('roundcube_opencloud.file' + (move ? 'move' : 'copy') + 'notice', 'confirmation');
    ...
    {
        this.req = this.set_busy(true, 'roundcube_opencloud.fileupdating');
    ...
    this.file_upload_size_check = function (files)
    {
        ...
        if (maxsize && files) {
            ...
            if (size > maxsize) {
                alert(rcmail.get_label('roundcube_opencloud.uploadsizeerror').replace('$size', roundcube_opencloud_file_size(maxsize)));
                return false;
            }
        }
    ...
    this.file_upload_progress_response = function (response)
    {
        ...
        if (param.total) {
            ...
            if (!param.done) { param.text = roundcube_opencloud_progress_str(param); }
    ...
    {
        var href = '?' + $.param({
            _task: 'roundcube_opencloud', _action: 'open', file: file, viewer: viewer == 2 ? 1 : 0,
        });
    ...
    this.folder_list_auth_dialog = function (label, driver)
    {
        ...
        args.buttons[this.t('roundcube_opencloud.save')] = function () {
            ...
            file_api.req = file_api.set_busy(true, 'roundcube_opencloud.authenticating');
            file_api.request('folder_auth', data, 'folder_auth_response');
        };

        args.buttons[this.t('roundcube_opencloud.cancel')] = function () {
        ...
        args.title = this.t('roundcube_opencloud.folderauthtitle').replace('$title', label);
    ...
    this.folder_auth_response = function (response)
    {
        ...
        roundcube_opencloud_dialog_close(this.open_dialog);
