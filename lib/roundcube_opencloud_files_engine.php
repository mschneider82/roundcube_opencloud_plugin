...
class roundcube_opencloud_files_engine
{
    ...
    public function __construct($plugin)
    {
        ...
        // Resolve credentials: global config > per-user preferences > IMAP fallback
        $userName = $plugin->rc->config->get('driver_webdav_username');
        ...
        // Final fallback to IMAP credentials
        if (empty($userName)) {
            $userName = $plugin->rc->user->get_username();
        }
        if (empty($password)) {
            $password = $plugin->rc->get_user_password();
        }
        ...
        if (!empty($spacesUrl)) {
            // Full spaces URL (e.g. OpenCloud): parse it into baseUri + prefix automatically
            $spacesUrl = str_replace('%u', $userName, $spacesUrl);
            $parsed = parse_url($spacesUrl);
            $port = isset($parsed['port']) ? ':' . $parsed['port'] : '';
            $baseUri = $parsed['scheme'] . '://' . $parsed['host'] . $port . '/';
            $prefix = rtrim($parsed['path'], '/');
        } else {
            $baseUri = $plugin->rc->config->get('driver_webdav_url');
            $prefix = $plugin->rc->config->get('driver_webdav_prefix');
            $baseUri = str_replace('%u', $userName, $baseUri);
            $prefix = str_replace('%u', $userName, $prefix);
        }
    ...
    /**
     * Check if per-user credentials are configured (either global config or user preferences)
     */
    public static function hasCredentials($plugin)
    {
        // Global config credentials
        if (!empty($plugin->rc->config->get('driver_webdav_username'))
            && !empty($plugin->rc->config->get('driver_webdav_password'))) {
            return true;
        }
